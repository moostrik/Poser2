"""Tests for the projection row's azimuth overlay geometry (eye vs bbox normalized azimuths)."""
...
def _frame(track_id: int, azimuth: float, bbox_azimuth: float | None = None) -> Frame:
    features: dict = {Azimuth: Azimuth.from_value(azimuth)}
    if bbox_azimuth is not None:
        features[BBoxAzimuth] = BBoxAzimuth.from_value(bbox_azimuth)
    return Frame(track_id=track_id, cam_id=0, features=features)


class BuildAzimuthMarksTest(unittest.TestCase):
    def test_both_azimuths_map_to_normalized_azimuths(self) -> None:
        marks = build_azimuth_marks({0: _frame(0, math.pi / 2, -math.pi / 2)})
        self.assertEqual(len(marks), 1)
        self.assertAlmostEqual(marks[0].eye_x, 0.25, places=5)
        self.assertAlmostEqual(marks[0].bbox_x, 0.75, places=5)

    def test_missing_bbox_azimuth_is_nan(self) -> None:
    ...
    def test_nan_azimuth_is_nan(self) -> None:
        marks = build_azimuth_marks({0: _frame(0, math.nan, 1.0)})
    ...
    def test_one_mark_per_person_with_a_pose(self) -> None:
        frames = {1: _frame(1, 0.0, 0.0), 0: _frame(0, 0.0, 0.0)}
        self.assertEqual([m.track_id for m in build_azimuth_marks(frames)], [0, 1])
