...
class LinesSettings(BaseSettings):
    enabled: Field[bool]  = Field(False, description="Enable Lines composition")
...
class Lines(Composition):
    ...
    def render(self, transport: Transport, white: np.ndarray, blue: np.ndarray) -> None:
        ...
        white   += np.where(vals_w < W.width, W.strength * g, 0.0).astype(white.dtype)
        ...
        blue    += np.where(vals_b < B.width, B.strength * g, 0.0).astype(blue.dtype)
