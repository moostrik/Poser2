...
class WindowSettings(BaseSettings):
    ...
    sync_threshold: Field[float] = Field(0.75, min=0.0, max=0.99,  step=0.01, description="Pair similarity from which windows open toward each other, fully at 1 (alike)")
...
class OverrideSettings(BaseSettings):
    ...
    detune_on:               Field[bool]  = Field(True,                                      description="Detune from here", newline=True)
    detune:                  Field[float] = Field(0.0,  min=0.0,  max=1.0,   step=0.01,      description="Blue's interval past white's (fraction)")
    window_on:               Field[bool]  = Field(True,                                      description="Window from here, no sync growth", newline=True)
    ...
    white_harmonic_on:       Field[bool]  = Field(True,                                      description="White's harmonic from here", newline=True)
    white_harmonic:          Field[float] = Field(0.0,  min=0.0,  max=1.0,   step=0.01,      description="White's harmonic drawbar")
    white_phase_on:          Field[bool]  = Field(True,                                      description="White's phase from here", newline=True)
    white_phase:             Field[float] = Field(0.0,  min=-1.0, max=1.0,   step=0.01,      description="White's phase (intervals, positive outward)")
    white_overtone_phase_on: Field[bool]  = Field(True,                                      description="White's overtone phase from here", newline=True)
    ...
    blue_harmonic_on:        Field[bool]  = Field(True,                                      description="Blue's harmonic from here", newline=True)
    blue_harmonic:           Field[float] = Field(0.0,  min=0.0,  max=1.0,   step=0.01,      description="Blue's harmonic drawbar")
    blue_phase_on:           Field[bool]  = Field(True,                                      description="Blue's phase from here", newline=True)
    blue_phase:              Field[float] = Field(0.5,  min=-1.0, max=1.0,   step=0.01,      description="Blue's phase (intervals, positive outward)")
    blue_overtone_phase_on:  Field[bool]  = Field(True,                                      description="Blue's overtone phase from here", newline=True)
