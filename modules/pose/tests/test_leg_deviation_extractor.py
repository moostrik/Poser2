...
def _settings(n_top: int = 2, dead_zone: float = 0.0) -> LegDeviationExtractorSettings:
    cfg = LegDeviationExtractorSettings()
    cfg.neutral_dead_zone = dead_zone
...
class LegDeviationExtractorTest(unittest.TestCase):
    def _legs(self, frame: Frame, n_top: int = 2, dead_zone: float = 0.0) -> float:
        return LegDeviationExtractor(_settings(n_top, dead_zone)).process(frame)[LegDeviation].value

    def test_the_range_starts_after_the_neutral_dead_zone(self) -> None:
        # From 30° to the knee's 90° top: a 60° bend sits half way.
        f = _frame(left_knee=math.radians(60.0))
        self.assertAlmostEqual(self._legs(f, n_top=1, dead_zone=30.0), 0.5, places=5)
        self.assertAlmostEqual(self._legs(_frame(left_knee=math.radians(30.0)), n_top=1, dead_zone=30.0), 0.0, places=5)
