...
class RecorderSettings(BaseSettings):
    ...
    features:       Field[list] = Field([Feature["BBox"], Feature["Points2D"]], description="Features to include in recording")

    # ── Controls ──────────────────────────────────────────────────────
    enabled:        Field[bool] = Field(True, widget=Widget.switch, description="Enable pose recording", newline=True)
