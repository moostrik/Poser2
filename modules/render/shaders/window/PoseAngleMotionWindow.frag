...
uniform float       output_aspect_ratio;
uniform float       display_range;  // max absolute value (typically 1.0 for normalized points)
...
void main() {
    ...
    for (int i = -2; i <= 2; i++) {
        ...
        // Map value to stream position: 0 at bottom, 1 at top (positive values only for motion)
        float curr_norm = clamp(check_curr_sample.r / display_range, 0.0, 1.0);
        float prev_norm = clamp(check_prev_sample.r / display_range, 0.0, 1.0);
    ...
    if (line_alpha > 0.001) {
        // Red-orange color for motion intensity
